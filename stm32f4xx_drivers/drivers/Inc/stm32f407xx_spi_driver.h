...
/*
 * Possíveis eventos de aplicação
 */
...
typedef struct                              //em pSPIx o usuario vai setar as informações desejadas que serão usadas pelo nosso driver para configurar os periféricos usando a estrutura SPI_Config, que já tem os Base Addrs definidos
{
	...
	uint8_t 		*pTxBuffer;             // !< To store the app. Tx buffer address > */ A partir daqui as variáveis dessa estrutura estrutura lidarão com interrupções
...
void SPI_IRQHandling(SPI_Handle_t *pHandle);                                               //Interrupção externa ativda por esse pino será gerenciada
