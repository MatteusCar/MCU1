...
void GPIO_IRQInterruptConfig(uint8_t IRQnumber, uint8_t  IRQEn)
{
	...
	//Cada periférico (SPI1, SPI2, GPIOA, GPIOB etc) tem um IRQ number que pode ser consultado no
	//manual de referência do usuário, mais especificamente no NVIC. Esse número IRQ serve para que o
	//processador saiba qual interrupção acionar, e para qual espaço de memória pular no momento em que ocorrer a interrupção
